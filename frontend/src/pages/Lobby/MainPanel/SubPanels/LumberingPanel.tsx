...
export default function LumberingPanel() {
  return (
    <Stack direction="column" sx={{ alignItems: "center" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, 120px)", justifyContent: "center", gap: 8, padding: "16px 4px 16px 12px", width: "100%" }}>
        {Array.from({ length: 16 }).map((_, index) => (
          <Button color="secondaryDark" variant="contained" sx={{ height: 120, width: 120, minWidth: 0, aspectRatio: 1, padding: 0 }}>
            {index === 0 ? "Hickory Lumber" : ""}
            {index === 1 ? "" : ""}
          </Button>
        ))}
      </div>
